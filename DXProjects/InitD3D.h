...
struct D3DContext
{
	...
	HRESULT InitD3D(HWND hWnd)
	{
		...
		desc.Width = width*DOWN_SAMPLE / 2;
		desc.Height = height*DOWN_SAMPLE / 2;
		m_pd3dDevices[0]->CreateTexture2D(&desc, data, &m_pMiniWorkBuffer[0]);
		m_pd3dDevices[0]->CreateTexture2D(&desc, data, &m_pMiniWorkBuffer[1]);
